...
public class NewJFrame extends javax.swing.JFrame {
    ...
    private void button1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_button1ActionPerformed
        ...
        new RecIntegral(upper, lower, steps); 
    ...
    for (int i = 0; i < model.getRowCount(); i++) {
        try {
            // Простое преобразование через toString() работает и для Number, и для String
            double upper = Double.parseDouble(model.getValueAt(i, 0).toString());
            double lower = Double.parseDouble(model.getValueAt(i, 1).toString());
            double steps = Double.parseDouble(model.getValueAt(i, 2).toString());
    ...
    private void writeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_writeActionPerformed
        // 1. Получаем коллекцию всех записей
        LinkedList<RecIntegral> allRecords = RecIntegral.GetCollection();

        // 2. Получаем модель таблицы
        DefaultTableModel model = (DefaultTableModel) jTable1.getModel();

        // 3. Проходим по каждому объекту в коллекции
        for (RecIntegral record : allRecords) {
            // 4. Берём значения из объекта record и добавляем в таблицу
            model.addRow(new Object[]{record.upp, record.down, record.step, ""});
        }
